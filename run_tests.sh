...
do 
    python3 ./run_tests.py --tolerance_higher $h_tol --test "h_tol" &
done

# Run test low_tol
for l_tol in 0.2 0.3 0.4 # lower tolerance
do 
    python3 ./run_tests.py --tolerance_lower $l_tol  --test "l_tol" &
done

# Run test land_value
for val in 0.4 0.5 0.6 # Land value
do 
    python3 ./run_tests.py --land_value $val --test "land_value" &
done

# Run test land_weight
for land_weight in 0.8 0.9 1.0 # Land weight
do 
    python3 ./run_tests.py   --neighbour_to_land_weight $land_weight --test "land_weight" &
done
